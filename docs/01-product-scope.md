...
本文是 5755 Android SDK 最小化 AAR 的权威产品范围定义,规定本产品的定位、一期能力白名单、公开 API 白名单、明确排除项、二期边界和技术基线。所有实现、接入文档、自检和验收均以本文边界为准;任何能力进入或退出产物,都必须先修订本文。

## 1. 产品定位

5755 Android SDK 最小化 AAR 是当前唯一上线产物。它只包含最小上线链路必需的业务能力和 SDK 基础能力,不继承完整 SDK 的 Manifest、依赖和 API 面。

- **单一交付产物**:接入方通过 Gradle 只依赖一个 AAR(`5755-sdk-release.aar`)。源码侧可以由 `sdk-core` 和 `sdk-ui` 装配到公开交付模块 `sdk`,但接入方不分别依赖它们。
...
| 能力 | 规格 |
| --- | --- |
| 协议告知 | 未完成协议确认不得完成登录。按安装级和协议版本级触发:新安装、清除数据、卸载重装或协议版本升级后展示;同一安装内协议已同意且版本未升级时不重复展示。 |
...
| 用户中心 H5 | 通过悬浮球入口打开最小用户中心 H5 容器,由 Android 系统 WebView 和最小 JS Bridge 承载。H5 只读取当前游戏小号上下文,把退出登录、切换游戏小号或账号失效类动作回传 SDK;不维护另一套当前账号,不提供切换 5755 账户。悬浮球只保留用户中心入口。 |
...
以下事项属于二期或可选范围,不进入当前产物。后续进入产物前,必须重新走能力白名单、公开 API、Manifest、依赖和接入自检评审:

| 能力 | 状态 |
| --- | --- |
| 真实用户中心 H5 联调(平台侧 H5 容器与 URL 契约) | 二期 |
| 真实平台配置、5755 账户、游戏小号、支付查询、充值回调线上链路联调 | 二期 |
| 生产 AAR 纯净化与运行模式隔离(移除模拟平台网关与 fixture 兜底) | 二期 |
| 设备标识采集或透传(OAID/Android ID) | 二期 |
| 广告归因 | 二期 |
| 第三方监控(monitorsdk/Kwai) | 二期 |
| 客服/IM/protobuf 消息 | 可选 |
| 平台 App 下载、安装、跳转 | 可选 |
| 活动、优惠券 | 可选 |
| 强更/版本更新门禁 | 可选 |
| 加速、连点器、native hook | 可选 |
| 文件上传、媒体选择、APK 处理、外部 App 跳转 | 可选 |

二期联调不引入完整包能力、不扩大公开 API、不改变 `account = 当前游戏小号 ID` 的语义。
...
| 用户中心 H5 | Android 系统 WebView + 最小 JS Bridge;Bridge 只允许读取当前游戏小号上下文并回传账号动作 |
