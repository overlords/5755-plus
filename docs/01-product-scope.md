...
| 支付 | 使用 SDK 自有支付容器和服务端订单/回调协议,不引入支付宝/微信原生 SDK。收银台内玩家选渠道付款时拉起微信/支付宝 App,属**支付域受限外跳例外**(仅收银台 WebView、仅白名单 scheme、`startActivity` 直拉 + 零 queries),见 §4.2/§4.3/§5、ADR-0014。支付 UI 展示和支付请求完全取自 `Order` 入参,订单绑定当前游戏小号、区服、角色、金额、商品和 CP 订单号;归属字段缺失时必须失败并输出诊断,禁止固定演示订单兜底。支付由玩家在游戏内点击购买后由游戏调用,SDK 不主动发起。 |
...
- 通用 H5 容器能力:文件上传、媒体选择、APK 下载/安装、外部 App 跳转
  - **唯一例外:支付域受限外跳**(经 §5 评审,见 ADR-0014)——仅**收银台** WebView、仅**微信/支付宝白名单 scheme**(`weixin://`、`alipays://` 及付款变体)、仅 `startActivity(VIEW)` 直拉,未安装走 `try/catch` 兜底(**不加 queries**、§4.3 不破)、不暴露公开 API、不引原生 SDK。**通用外跳仍永久排除**。
...
> **支付域外跳不破本节(零 queries)**:收银台拉起微信/支付宝走 `startActivity(VIEW)` 直拉 + `ActivityNotFoundException` 兜底——Android 11 package visibility 只过滤「查询」(`resolveActivity`/`queryIntentActivities`)、不阻止「直接启动」,故**无需声明 `<queries>`**。支付宝/微信 queries 仍在排除之列,Manifest 零变化(见 §4.2 例外、ADR-0014)。
...
| 文件上传、媒体选择、APK 处理 | 可选·永久排除(§4.2) |
| 外部 App 跳转 | **通用外跳永久排除(§4.2);支付域受限例外**(仅收银台拉起微信/支付宝、零 queries)已过 §5 评审,见 §4.2 例外、ADR-0014 |
