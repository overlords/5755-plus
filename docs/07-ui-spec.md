...
- **动效**:界面出现/消失无进出场动画(与旧实现一致);允许新增轻微淡入淡出(含 **WebView 就绪淡入**,§1.13),**禁止**弹跳、循环、装饰性动效(故 WebView 加载态用静态占位,**不用 spinner 转圈**)。计时行为仅限:验证码 60s 倒计时、toast 1600ms、自动进入提示 1800ms。
...
### 1.13 WebView 加载态(2026-06-13 决定;同日修订:加载中态占位由纯文字改为品牌静帧图)

两个远程 WebView(用户中心 §11.2、协议网页层 §2)加载远程页时,在页面首次绘制前(**A 层**:网络拉取 + 初次渲染,纯白屏)以**静态占位**覆盖,页面就绪后**淡入**揭示;**无 spinner/循环动效**(§1.11)。

- **占位(加载中态)**:`WEAK` 中性底铺满 WebView 区;居中放 **5755 品牌静帧图**(Gold Orbit 静帧,160dp 宽、3:2 比例、`fitCenter`)——金色轨道环 + 斜体「5755」+ 三个金点,**冻结为静态单帧,不旋转/不闪烁/不循环**(§1.11)。**无文字提示**(品牌图即占位;文字交由 SPA 骨架屏 B 层接力),故不叠加独立「加载中」文字。
- **资产**:`res/drawable-nodpi/m5755_web_loading.png`,由设计系统「5755 Gold Orbit」动效 SVG **去动画取基准帧**栅格化派生(去掉源图的「加载中」文字与循环动画,单张 3x 高清交 `ImageView` 缩放;源 SVG 不入库)。品牌字以**位图像素**呈现而非字体文件,不与 §1.11「原生层不内嵌字体」冲突。
...
- **WebView**(铺满抽屉):系统 WebView;`javaScriptEnabled=true`、`javaScriptCanOpenWindowsAutomatically=false`、文件访问全关;`overScrollMode=NEVER`;**UserAgent 追加 `M5755Sdk/<版本>`**;`WebViewClient` 拦截 http/https **站内加载不外跳**;JS 桥名 **`UserCenter`**;**加载态见 §1.13**(远程页静态占位 + 就绪淡入 + 失败重试;回退页瞬时不显占位)。
