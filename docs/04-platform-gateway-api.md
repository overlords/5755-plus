...
| `cpOrderId` | 游戏订单号(游戏侧自保唯一,平台不强制约束);原样返回,游戏侧用于归属与金额交叉校验,**不作去重键**。 |
...
- **游戏服务端只按 `orderId` 幂等去重**(`orderId` = `orders` 表主键、全局唯一,是唯一防线);`cpOrderId` 平台不保证唯一,**不能作去重键**。`cpOrderId` / `amount` / `account` 仅作一致性交叉校验(不符则拒绝发货)。
