...
**调用方与鉴权**:本端点既被 SDK 调用(诊断登录态、SDK keyId 签名),也被**游戏服务端**调用(校验玩家上报的小号 `token` 真伪)。游戏服务端用独立 `serverKey` 按 §1.3 同样的 HMAC-SHA256 + 时间戳防重放签名(`X-M5755-Key-Id` 填 `serverKeyId`),平台按 `keyId` 区分主体;两类调用者共用本端点、同一 `token` 语义。见 ADR-0016。
...
- **平台**用该游戏的 **`serverSecret`** 签;若该游戏已发放 `serverKey` 则用之,dev 路径回退默认 `dev-server-key`。
