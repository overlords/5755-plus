...
| `money` | 订单金额(历史兼容)。 |
| `pay_money` | 实付金额。 |
| `sign` | 充值回调签名,口径见下「签名口径(MD5)」。 |

### 签名口径(MD5)

充值回调 `sign` 用 **MD5**(出站独立口径,**非**入站 §1.3 的 HMAC-SHA256;2026-06-15 支付宝沙箱实战实证、v2 定稿):

1. 取回调体除 `sign` 外的全部字段,按字段名**字典序**(UTF-8 字节序)升序;
2. 逐对拼 `键=值&`(每对后都带 `&`,**含最后一对**);
3. 末尾追加 `key=<密钥>`(字面 `key=` + 平台↔游戏服务端约定密钥,**无尾随 `&`**);
4. 对整串 UTF-8 字节做 **MD5**,输出**十六进制小写**。

即 `MD5("k1=v1&k2=v2&…&kn=vn&key=<secret>")` → hex;`sign` 放回调体平级字段(非 HTTP 头)。密钥由平台与游戏服务端带外约定(dev 联调默认 `m5755-dev-callback-secret-v1`),游戏服务端按同口径复算比对验签。

> 出站回调**不复用**入站 HMAC-SHA256(§1.3 / `X-M5755-Signature` 头):入站是 SDK→平台、出站是平台→游戏服务端,两条独立链路。将来游戏方若要求,可双边(平台 `callbackSign` + 游戏服务端)同步升级 HMAC-SHA256;v2 定稿 MD5(沙箱 mock 游戏服务端已按此口径验签通过)。
