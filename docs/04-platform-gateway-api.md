...
| `principal_not_allowed` | 验签通过但调用主体越权:游戏服务端 `serverKey` 调了登录态校验(`GET subaccount-sessions`)以外的端点,**或 serverKey 与被查游戏不符**(其 `game_id` 与请求 `gameId` 不一致 / 为空)(ADR-0016) | **面向游戏服务端调用方,SDK keyId 永不触发**;HTTP 403,与 `signature_invalid`(验签层失败)区分 | 否 |
...
| `X-M5755-Timestamp` | 是 | 请求发起的 Unix 时间戳(秒),参与签名与时效窗口(防过期,见下)。 |
...
- **时效窗口 ≠ 完整防重放**:±300 秒窗口只限制重放的**时间暴露面**;canonical 串**不含 nonce**、服务端**不维护一次性 seen-cache**,故**窗口内对同一已签名请求的逐字节重放仍可通过**。在 v2 威胁模型(玩家只能重放自己抓到的请求,且无签名密钥不能篡改)下此残留可接受。**面向网络级攻击者(重放他人抓获的请求)的真·防重放(请求 nonce + 服务端一次性缓存)列为后续硬化项,不在 v2 范围。**
...
**游戏服务端调用(`serverKey`)**:游戏服务端调用本网关面端点(目前仅登录态校验 `GET subaccount-sessions`,见 §2.7.2)时,用独立的 **`serverKey`**(每游戏 `serverKeyId + serverSecret`,平台后台发放)按上述**同一 HMAC-SHA256 + 时间戳防重放**规则签名;`X-M5755-Key-Id` 填 `serverKeyId`,平台据 `keyId` 区分调用主体(SDK keyId 焊 AAR / `serverKeyId` 发放给游戏服务端),选对应密钥验签。`serverKey` **不焊 AAR**(游戏服务端是独立主体);其 `serverSecret` 同时用于充值回调签名(§4),使游戏服务端对接「登录态校验 + 充值回调」**只学一套 HMAC-SHA256 签名**。**`serverKey` 绑定其所属游戏**(`signing_keys.game_id`,migration `0012`);平台在登录态校验(§2.7.2)校验来访 `serverKey` 的 `game_id` 与被查 `gameId` 一致,不符 **403 `principal_not_allowed`**(堵跨租户探测)。SDK keyId 焊 AAR、全局,不绑游戏(`game_id=''`)。见 ADR-0016。
...
| `userCenterUrl` | 用户中心 H5 **完整地址**(平台远程页,以主账户为核心)。**是完整 URL,非域**(区别于上面两个域字段);SDK 仅在末尾追加 `#token=<platformToken>` 加载(**fragment 而非 query**:`#` 段不发往服务器,不进访问日志/代理缓存/Referer,与 §1.4「敏感凭据禁止进入 query string」同口径;页面 JS 凭 `location.hash` 取用),见 06 §5、ADR-0018。可选:平台未配置时省略(`omitempty`),SDK 缺省回退内置最小页。 |
...
**调用方与鉴权**:本端点既被 SDK 调用(诊断登录态、SDK keyId 签名),也被**游戏服务端**调用(校验玩家上报的小号 `token` 真伪)。游戏服务端用独立 `serverKey` 按 §1.3 同样的 HMAC-SHA256 + 时间戳防重放签名(`X-M5755-Key-Id` 填 `serverKeyId`),平台按 `keyId` 区分主体;两类调用者共用本端点、同一 `token` 语义。`serverKey` 绑定其所属游戏(`signing_keys.game_id`),平台校验来访 `serverKey` 的 `game_id` 与被查 `gameId`(请求参数)一致,**不符返 403 `principal_not_allowed`**(游戏 A 的 serverKey 不得探测游戏 B 的登录态);SDK keyId 全局、不做 game 比对。见 ADR-0016。
...
| `cpOrderId` | body | 是 | 游戏订单号;非空,**长度不超过 128,服务端强制校验**;平台按 `(gameId, cpOrderId)` 创建侧幂等(见业务规则)。 |
| `amount` | body | 是 | 金额,单位元,字符串金额;**两位小数,正则 `^\d+\.\d{2}$`(如 `"328.00"`),必须大于 0**(与 `roleRechargeAmount` 同口径,见 05),**服务端强制校验**(`0`、`0.00`、负数、非两位小数格式均拒绝)。 |
...
| 字段 | 说明 |
| --- | --- |
| `orderId` | 平台订单号(`orders` 表主键、全局唯一);充值回调与订单查询沿用同一字段。 |
...
- **创建侧 `(gameId, cpOrderId)` 幂等(防重放 / 防重复下单)**:平台按 `(gameId, cpOrderId)` 强制唯一(DB `UNIQUE(game_id, cp_order_id)`)。同键再次创建时——仍 `待支付` 且金额/商品/区服/角色等归属字段与原单**一致** → **返回已存在订单**(同 `orderId`/`paymentUrl`,不新建);字段**不一致**(同键改金额等)→ 拒,`reason=order_invalid`;已 `已支付` → 拒,`reason=order_invalid`(已支付订单不可重复创建)。**跨游戏不保证唯一**;**本幂等只作用于创建,充值回调去重键仍固定为 `orderId`**(见第 4 节)。见 ADR-0020。
...
| 字段 | 说明 |
| --- | --- |
| `paymentStatus` | 支付状态,完整取值:`待支付` / `已支付` / `支付失败`。 |
| `callbackStatus` | 充值回调(服务端)状态,仅用于诊断,完整取值:`未投递` / `投递中` / `已确认` / `投递失败` / `无回调地址`。 |
...
回调字段(最终形态,共 10 字段,全 camelCase):

| 字段 | 说明 |
| --- | --- |
| `account` | 当前游戏小号 ID。 |
| `orderId` | 平台订单号(`orders` 表主键、全局唯一);**充值回调幂等去重的唯一键**(见下「幂等与防重放」)。 |
| `cpOrderId` | 游戏订单号;平台按 `(gameId, cpOrderId)` **创建侧幂等**(§2.9.1)、**跨游戏不保证唯一**;原样返回,游戏侧用于归属与金额交叉校验,**回调去重不以它为键**(去重键固定 `orderId`)。 |
| `amount` | 订单金额(应付金额),单位元,字符串金额。 |
| `payAmount` | 实付金额;v2 恒等订单金额 `amount`(无折扣/券/余额),保留作前向兼容缝(ADR-0012)。 |
| `commodity` | 商品名称。 |
| `serverId` | 区服 ID。 |
| `serverName` | 服务器名称。 |
| `serverKeyId` | 平台签名所用 `serverKey` 的非密标识;游戏服务端据此选对应 `serverSecret` 验签(ADR-0016)。进签名串。 |
| `sign` | 充值回调签名,口径见下「签名口径(HMAC-SHA256)」。 |

### 签名口径(HMAC-SHA256)

充值回调 `sign` 用 **HMAC-SHA256**(以游戏的 `serverSecret` 为密钥;经 ADR-0016 由 MD5 改为 HMAC-SHA256,与入站 §1.3 同算法族,统一游戏服务端面签名):

1. 取回调体除 `sign` 外的全部字段(**含 `serverKeyId`**),按字段名**字典序**(UTF-8 字节序)升序;
2. 逐对拼 `键=值&`(每对后都带 `&`,**含最后一对**);
3. 以游戏的 **`serverSecret`** 为密钥,对该串 UTF-8 字节做 **HMAC-SHA256**,输出**十六进制小写**。

即 `HMAC-SHA256(serverSecret, "k1=v1&k2=v2&…&kn=vn&")` → hex;`sign` 放回调体平级字段(非 HTTP 头)。**注意**:HMAC 的 `serverSecret` 是密钥参数、**不拼进串**(MD5 旧口径的末尾 `key=<密钥>` 已移除)。`serverSecret` 是每游戏的服务端密钥(`serverKey`,见 §1.3 / ADR-0016;dev 联调默认 `serverKeyId='dev-server-key'`、secret `m5755-dev-callback-secret-v1`)。

- **平台**按 `gameId` 取该游戏**最新 active** `serverKey`(`signing_keys.game_id` 选密钥,实现为 `signingServerKey`)、用其 `serverSecret` 签;该游戏未配 per-game `serverKey` 时回退默认 `dev-server-key`。
- **`serverKeyId` 作为回调体字段之一一并进签名串**(参与上述字典序拼接),标明本次签名所用密钥。
- **游戏服务端**据回调体的 `serverKeyId` 选对应 `serverSecret`,按同口径复算比对验签。

> 充值回调与入站 §1.3 **同用 HMAC-SHA256**,但**用途与密钥不同**:入站是 SDK→平台(SDK keyId、焊 AAR)、出站回调是平台→游戏服务端(`serverKey`、每游戏发放),两条独立链路、各自密钥。ADR-0016 统一为 HMAC-SHA256 后,游戏服务端对接「登录态校验 + 充值回调」**只学一套签名**。

### 幂等与防重放

- **游戏服务端只按 `orderId` 幂等去重**(`orderId` = `orders` 表主键、全局唯一,是唯一防线);`cpOrderId` 平台仅在**创建侧**按 `(gameId, cpOrderId)` 幂等(§2.9.1)、**跨游戏不保证全局唯一**,故**回调去重不以它为键**。`cpOrderId` / `amount` / `account` 仅作一致性交叉校验(不符则拒绝发货)。
- **充值回调无时间戳/防重放窗口**:平台自愈巡检(`RedeliverPendingCallbacks`)会重投与首发**逐字节相同**的陈旧但有效回调,加防重放窗口会与自愈机制互斥;`orderId` 幂等去重即是重复投递与重放的唯一防线。
- **出入站不对称**:入站登录态校验(`GET subaccount-sessions`)仍走 §1.3 的 **±300 秒**时间戳窗口;出站回调不设窗口。出入站不对称是自愈设计的必然,非疏漏。

### serverKey 轮换

v2 默认**每游戏单把 active `serverKey`**。回调体携带非密 `serverKeyId` 标明本次签名所用密钥,使「换密钥」从契约变更**降为配置操作**(签新发的密钥、回调体填新 `serverKeyId`,游戏服务端据它选密钥验签),与入站 `X-M5755-Key-Id` 的轮换能力对称。真实游戏方的 per-game `serverKey` 发放/联调归后续 issue;本批 dev 路径用默认 `dev-server-key`。
