...
1. 取回调体除 `sign` 外的全部字段,按字段名**字典序**(UTF-8 字节序)升序;
2. 逐对拼 `键=值&`(每对后都带 `&`,**含最后一对**);
3. 以游戏的 **`serverSecret`** 为密钥,对该串 UTF-8 字节做 **HMAC-SHA256**,输出**十六进制小写**。

即 `HMAC-SHA256(serverSecret, "k1=v1&k2=v2&…&kn=vn&")` → hex;`sign` 放回调体平级字段(非 HTTP 头)。**注意**:HMAC 的 `serverSecret` 是密钥参数、**不拼进串**(MD5 旧口径的末尾 `key=<密钥>` 已移除)。`serverSecret` 是每游戏的服务端密钥(`serverKey`,见 §1.3 / ADR-0016;dev 联调默认 `m5755-dev-callback-secret-v1`),游戏服务端按同口径复算比对验签。
