...
- **换绑手机**(标题 `换绑手机`):说明 hint;`Input` 新手机号(11 位校验,口径同 `07 §3`);内嵌按钮输入行 验证码 + `发送验证码`(60s 倒计时);主按钮 `确认换绑`。成功 → 返回主页刷新 + toast `换绑成功`。
- **修改密码**(标题 `修改密码`):说明 hint(短信验证身份);内嵌按钮输入行 验证码 + `发送验证码`(发往已绑手机);`Input` 新密码(密码型 + 显示切换);主按钮 `确认修改`。成功 → toast `密码已修改` → 调 `session_invalid` 回登录窗。
...
- **文案清单**:`当前小号`、`切换`、`账号安全`、`绑定手机`、`修改密码`、`实名认证`、`已实名`、`未实名`、`我的`、`充值订单`、`退出登录`、`换绑手机`、`发送验证码`、`确认换绑`、`确认修改`、`换绑成功`、`密码已修改`、`暂无充值订单`、`请在游戏内打开`、`重试`。
