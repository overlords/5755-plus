...
- 包名为 `com.m5755.sdk.ui.sample`;新玩家首登场景须先清空该包数据。
...
- 真实登录成功后,本地 `shared_prefs/m5755_operate_min.xml` 必须出现 `platform_account_id`、`platform_token` 和当前游戏小号 `account`。
  - **at-rest 明文为 v2 显式范围决定(非疏漏)**:会话令牌(`platform_token`/`sub_token`)以明文存于 `MODE_PRIVATE` prefs,理由是最小化 AAR + 01 依赖白名单、`MODE_PRIVATE` 已沙箱到本 App UID、存的是玩家自己的令牌、服务端有 `expiresAt` + 踢号可撤销。**残留风险**:设备丢失/共享、root 恶意软件或 `adb backup` 提取下,主账户令牌外泄可经用户中心改密/换绑导致**主账户接管**;此风险在「威胁模型不含 root/丢失设备」前提下被接受。**后续硬化项**(列入范围外、不阻塞 v2):Android Keystore(AES/GCM)包裹双令牌、宿主 `allowBackup=false`。该决定立档见 `adr/0019`。
...
| 4 | 真实小号选择 | 新玩家实名通过后展示真实游戏小号列表,不出现演示小号 | `assets/acceptance/02_subaccount_picker.png`,日志 `sub_account real_list count=N` |
